...
import ConversionTableBody from './ConversionTableBody';

const ConversionRowTable = (props) => {
    const {
    ...
    return (
        <Table responsive>
            ...
            <tr>
                ...
                <th>{getFormattedMessage("expense.input.amount.label")}</th> */}
            ...
            {updateProducts && updateProducts.length ? updateProducts.map((singleProduct, index) => {
                return <ConversionTableBody singleProduct={singleProduct} key={index} index={index} updateProducts={updateProducts} warehouse={warehouse}
...
export default ConversionRowTable;
